...
Функция СхемаКомпоновкиОбщая(ОписаниеКолонок)
	...
	ЗаполнитьСортировку(Схема);
	ЗаполнитьПредопределенныеПараметры(Схема); 
	ЗаполнитьПредопределенноеУсловноеОформление(Схема);
	
	Возврат Схема;
	
КонецФункции    

Процедура ЗаполнитьПредопределенноеУсловноеОформление(Схема)
	
	ЭлементыУсловноеОформление = Схема.НастройкиПоУмолчанию.УсловноеОформление.Элементы;
	
	// Фон помессенных на удаления ошибок
...
Процедура ЗаполнитьСортировку(Схема)
...
Функция ПараметрыПолученияЗаголовковИзПараметровМонитора(ПараметрыМонитора)
	...
	Если ПараметрыМонитора.Свойство("НастройкиСКДДляДокументаПоВсемОшибкамСтрока") 
