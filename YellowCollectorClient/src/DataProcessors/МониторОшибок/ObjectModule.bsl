...
Процедура ПодготовитьМониторКРаботеПриНеобходимости(ПараметрыМонитора) Экспорт
	...
	// TODO: Добавить в ново api метод, который возвращает структуру отчета обшибке 
	ДополнитьПараметрыМонитораИформациейИзЗаголовков(ПараметрыМонитораИзменяемые, ПолучитьЗаголовкиВсехОтчетовОбОшибках());
...
Процедура ДополнитьПараметрыМонитораИформациейИзЗаголовков(ПараметрыМонитора, ЗаголовкиВсехОтчетов)
	ПараметрыМонитора.Вставить("ЭталонныйЗаголовокОтчетаДляПостроенияКолонокТаблицы", ЗаголовкиВсехОтчетов[0]);
КонецПроцедуры
...
Процедура ПодготовитьМониторКРаботе(Знач Параметры = Неопределено)
	...
	СтруктураКолонок = СтруктураКолонокТаблицыИзЗаголовкаОшибки(Параметры["ЭталонныйЗаголовокОтчетаДляПостроенияКолонокТаблицы"]);
...
Процедура ИсключитьПоляИзМакетовПолейСхемы(ОписаниеИсключаемыхПолей, Схема)
	...
	Для Каждого ОписаниеПолей Из ОписаниеИсключаемыхПолей Цикл 
		Если ОписаниеПолей.Найти(Новый ПолеКомпоновкиДанных("ИД")) = Неопределено Тогда
...
Функция СтруктураКолонокТаблицыИзЗаголовкаОшибки(ЗаголовокиОшибки)
	...
	Для Каждого Заголовок Из ЗаголовокиОшибки Цикл
		
		ОписаниеКолонки = Новый Структура("ИмяКолонки, Заголовок, ТипСтрокой");
		СоответствиеЗаголовка = СоотвествиеИмен.Получить(Заголовок.Ключ);
		Если СоответствиеЗаголовка = Неопределено Тогда
			
			ОписаниеКолонки.ИмяКолонки = Заголовок.Ключ;
			ОписаниеКолонки.Заголовок = Заголовок.Ключ;
			ОписаниеКолонки.ТипСтрокой = "Строка";
		...
		ОбратноеСоответствиеИмен.Вставить(ОписаниеКолонки.ИмяКолонки, Заголовок.Ключ);
