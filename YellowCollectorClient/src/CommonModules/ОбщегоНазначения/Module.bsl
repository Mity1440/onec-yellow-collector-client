...
Функция ПеревернутьМассив(ИсточникМассив) Экспорт
	
	Результат = Новый Массив();
 	
 	ИндексПоследний = ИсточникМассив.ВГраница();
	Для Ид = -ИндексПоследний По 0 Цикл
		Результат.Добавить(ИсточникМассив[-Ид]);	
	КонецЦикла;
...
//  ПараметрыЗаписиJSON - ПараметрыЗаписиJSON - 
// 
// Возвращаемое значение:
//  Строка - JSON-строка.
//
