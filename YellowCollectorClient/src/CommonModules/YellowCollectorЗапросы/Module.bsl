// @strict-types

#Область ПрограммныйИнтерфейс

// Сервер доступен.
// 
// Возвращаемое значение:
//  Булево - Сервер доступен
Функция СерверДоступен() Экспорт
	
	СерверДоступен = Истина;
	
	Заголовки = Новый Соответствие;
	Заголовки.Вставить("Content-Type", "application/json");
	
	ДополнительныеПараметры = Новый Структура("Заголовки", Заголовки);
	
	Попытка
		КоннекторHTTP.Get(URlДляПроверкиДоступностиСервера(), Неопределено, ДополнительныеПараметры);
	Исключение
		СерверДоступен = Ложь;
	КонецПопытки;
	
	Возврат СерверДоступен;
	
КонецФункции

// Получить заголовки всех отчетов об ошибках по параметрам.
// 
// Возвращаемое значение:
//  Соответствие, Структура - Получить заголовки всех отчетов об ошибках по параметрам
Функция ПолучитьЗаголовкиВсехОтчетовОбОшибкахПоПараметрам(Знач ВходящиеПараметры = Неопределено) Экспорт
	
	Заголовки = Новый Соответствие;
	Заголовки.Вставить("Content-Type", "application/json");
	
	ПараметрыЗапроса = ПараметрыЗапросаПолучитьЗаголовкиВсехОтчетовОбОшибкахШаблон();
	ЗаполнитьЗначенияСвойств(ПараметрыЗапроса, ВходящиеПараметры);
	
	ДополнительныеПараметры = Новый Структура("Заголовки", Заголовки);
	
	Возврат КоннекторHTTP.PostJson(URlДляПолученияСпискаОшибок(), ПараметрыЗапроса, ДополнительныеПараметры);
	
КонецФункции

Функция СтатистикаПоОшибкамОбщая(Знач ВходящиеПараметры = Неопределено) Экспорт
	
	Заголовки = Новый Соответствие;
	Заголовки.Вставить("Content-Type", "application/json");
	
	ПараметрыЗапроса = ПараметрыЗапросаПолучитьЗаголовкиВсехОтчетовОбОшибкахШаблон();
	ЗаполнитьЗначенияСвойств(ПараметрыЗапроса, ВходящиеПараметры);
	
	ДополнительныеПараметры = Новый Структура("Заголовки", Заголовки);
	
	Возврат КоннекторHTTP.PostJson(URlДляСтатистикаПоОшибкамОбщая(), ПараметрыЗапроса, ДополнительныеПараметры);
	
	
КонецФункции

// Получить маппинг полей
// 
// Возвращаемое значение:
//  Соответствие, Структура - Получить заголовки всех отчетов об ошибках по параметрам
Функция ПолучитьМаппингПолейЗапросаПолученияСпискаОшибок() Экспорт
	
	Заголовки = Новый Соответствие;
	Заголовки.Вставить("Content-Type", "application/json");
	
	ДополнительныеПараметры = Новый Структура("Заголовки", Заголовки);
	
	Возврат КоннекторHTTP.PostJson(URlДляПолученияМаппингаПолейЗапросаПолученияСпискаОшибок(), 
	                               Новый Структура, 
	                               ДополнительныеПараметры);
	
КонецФункции

// Получить маппинг полей
// 
// Возвращаемое значение:
//  Соответствие, Структура - Получить заголовки всех отчетов об ошибках по параметрам
Функция ПолучитьМаппингПолейЗапросаОбщаяСтатистика() Экспорт
	
	Заголовки = Новый Соответствие;
	Заголовки.Вставить("Content-Type", "application/json");
	
	ДополнительныеПараметры = Новый Структура("Заголовки", Заголовки);
	
	Возврат КоннекторHTTP.PostJson(URlДляПолученияМаппингаПолейЗапросаОбщаяСтатистика(), 
	                               Новый Структура, 
	                               ДополнительныеПараметры);
	
КонецФункции


// Получить детали отчета об ошибке по ИД.
// 
// Параметры:
//  ИД - Строка - 
// 
// Возвращаемое значение:
//  Соответствие, Структура - 
Функция ПолучитьДеталиОтчетаОбОшибкеПоИД(ИД) Экспорт
	
	Заголовки = Новый Соответствие;
	Заголовки.Вставить("Content-Type", "application/json");
	
	Возврат КоннекторHTTP.PostJson(URLДляПолученияДеталейОтчетаОбОшибках(ИД), Новый Структура);
	
КонецФункции  

// Получить детали отчета об ошибке по ИД.
// 
// Параметры:
//  ИД - Строка - 
// 
// Возвращаемое значение:
//  Соответствие, Структура - 
Функция ПометитьНаУдалениеЗаголовокОтчетаПоИД(ИД) Экспорт
	
	Заголовки = Новый Соответствие;
	Заголовки.Вставить("Content-Type", "application/json");
	
	Возврат КоннекторHTTP.PostJson(URLДляПометкиУдаленияОтчетаОбОшибкахПоИД(ИД), Новый Структура);
	
КонецФункции

#КонецОбласти

#Область СлужебныеПроцедурыИФункции

Функция ПолучитьЗапросник() Экспорт
	Возврат Обработки.ЗапросникYellowCollector.Создать();
КонецФункции

#Область ФормированиеURL

// Url для получения списка ошибок.
// 
// Возвращаемое значение:
//  Строка - 
Функция URlДляПолученияМаппингаПолейЗапросаПолученияСпискаОшибок()
	
	Возврат ПолучитьЗапросник()
	        .УстановитьМаршрут(Справочники.МаршрутыYellowCollector.V1_ErrorListMapping)
	        .Иницизировать()
	        .СформироватьURL();
	
КонецФункции    
		
// Url для получения списка ошибок.
// 
// Возвращаемое значение:
//  Строка - 
Функция URlДляПолученияМаппингаПолейЗапросаОбщаяСтатистика()
	
	Возврат ПолучитьЗапросник()
	        .УстановитьМаршрут(Справочники.МаршрутыYellowCollector.V1_BaseAggregationMapping)
	        .Иницизировать()
	        .СформироватьURL();
	
КонецФункции

// Url для получения списка ошибок.
// 
// Возвращаемое значение:
//  Строка - 
Функция URlДляПолученияСпискаОшибок()
	
	Возврат ПолучитьЗапросник()
	        .УстановитьМаршрут(Справочники.МаршрутыYellowCollector.V1_ErrorList)
	        .Иницизировать()
	        .СформироватьURL();
	
КонецФункции

// Url для получения списка ошибок.
// 
// Возвращаемое значение:
//  Строка - 
Функция URlДляСтатистикаПоОшибкамОбщая()
	
	Возврат ПолучитьЗапросник()
	        .УстановитьМаршрут(Справочники.МаршрутыYellowCollector.V1_BaseAggregation)
	        .Иницизировать()
	        .СформироватьURL();
	
КонецФункции

// Url для проверки доступности сервера.
// 
// Возвращаемое значение:
//  Строка -
Функция URlДляПроверкиДоступностиСервера()
	
	Возврат ПолучитьЗапросник()
	        .УстановитьМаршрут(Справочники.МаршрутыYellowCollector.Ping)
	        .Иницизировать()
	        .СформироватьURL();
	
КонецФункции

// Url для детальной информации отчета об ошибках
// 
// Возвращаемое значение:
//  Строка -
Функция URLДляПолученияДеталейОтчетаОбОшибках(ИД)
	
	Параметры = Новый Структура;
	Параметры.Вставить("ИД", ИД);
	
	Возврат ПолучитьЗапросник()
	        .УстановитьМаршрут(Справочники.МаршрутыYellowCollector.V1_ErrorDetails)
	        .Иницизировать(Параметры)
	        .СформироватьURL();
	        
КонецФункции

Функция URLДляПометкиУдаленияОтчетаОбОшибкахПоИД(ИД)
	
	Параметры = Новый Структура;
	Параметры.Вставить("ИД", ИД);
	
	Возврат ПолучитьЗапросник()
	        .УстановитьМаршрут(Справочники.МаршрутыYellowCollector.V2_ErrorMarkedForDeletion)
	        .Иницизировать(Параметры)
	        .СформироватьURL();
	
КонецФункции

#КонецОбласти

#КонецОбласти

#Область СлужебныеПроцедурыИФункции

// Параметры запроса получить заголовки всех отчетов об ошибках шаблон.
// 
// Возвращаемое значение:
//  Структура - Параметры запроса получить заголовки всех отчетов об ошибках шаблон:
// * from - Дата - 
// * to - Дата - 
// * includeDeletedMarkReports - Булево - 
Функция ПараметрыЗапросаПолучитьЗаголовкиВсехОтчетовОбОшибкахШаблон()
	
	Шаблон = Новый Структура;
	Шаблон.Вставить("from");
	Шаблон.Вставить("to");
	Шаблон.Вставить("includeDeletedMarkReports");
	
	Возврат Шаблон;
	
КонецФункции

#КонецОбласти

